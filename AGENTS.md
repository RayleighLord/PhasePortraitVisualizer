...
- Provide a multi-seed action that launches a small family of trajectories from random points in the visible window.
...
- The multi-seed action should use random points distributed across the visible grid rather than points on a circle.
- The default reset/reference example should be the nonlinear limit-cycle example.
- The default reset/reference example should open with no trajectories already drawn.
- The default limit-cycle setup should not appear as one of the loadable preset examples.
...
- When tuning the animated flow, bias slightly toward faster motion if the cue becomes too subtle, but keep it readable rather than frantic.
...
- The multi-seed button should create a family of trajectories from random points in the visible window.
